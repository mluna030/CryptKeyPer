...
pub mod merkle;
pub mod wots;
pub mod wots_optimized;
pub mod xmss_optimized;
pub mod xmss_mt;
...
mod tests {
    ...
    #[test]
    fn test_xmss_creation() -> Result<()> {
        let xmss = Xmss::new(10)?;
        assert_eq!(xmss.remaining_signatures(), 1 << 10);
        Ok(())
    }

    #[test]
    fn test_xmss_sign_verify() -> Result<()> {
        let mut xmss = Xmss::new(10)?;
        let message = b"test message";
        let message_hash = crate::hash_function::Sha256HashFunction::hash(message);
        let message_hash_array: [u8; 32] = message_hash.try_into().unwrap();

        let signature = xmss.sign(&message_hash_array)?;
        let is_valid = Xmss::verify(&message_hash_array, &signature, &xmss.public_key)?;
        assert!(is_valid);
        Ok(())
    }

    #[test]
    fn test_xmss_sign_multiple() -> Result<()> {
        let mut xmss = Xmss::new(2)?;
        let message1 = b"message one";
        let message1_hash: [u8; 32] = crate::hash_function::Sha256HashFunction::hash(message1).try_into().unwrap();
        xmss.sign(&message1_hash)?;

        let message2 = b"message two";
        let message2_hash: [u8; 32] = crate::hash_function::Sha256HashFunction::hash(message2).try_into().unwrap();
        xmss.sign(&message2_hash)?;

        assert_eq!(xmss.current_index(), 2);
        Ok(())
    }

    #[test]
    fn test_xmss_no_more_signatures() -> Result<()> {
        let mut xmss = Xmss::new(1)?; // Only 2 signatures available
        let message = b"test";
        let message_hash: [u8; 32] = crate::hash_function::Sha256HashFunction::hash(message).try_into().unwrap();

        xmss.sign(&message_hash)?;
        xmss.sign(&message_hash)?;

        let result = xmss.sign(&message_hash);
        assert!(result.is_err());
        match result.unwrap_err() {
            CryptKeyperError::NoMoreSignatures => assert!(true),
            _ => assert!(false, "Expected NoMoreSignatures error"),
        }
        Ok(())
    }
}